...
public class DeleteUseCase<I, DE, E> : IUseCaseAsync<DE, E, UseCaseErrorImpl>
    where I : notnull
    where E : notnull, IIdentifiable<I>
    where DE : notnull, IIdentifiable<I>
{
    private readonly IDeleterAsync<I, E> _deleter;
    private readonly IBusinessValidationService<DE>? _validator;

    /// <summary>
    /// Inicializa una nueva instancia de la clase <see cref="DeleteUseCase{I, DE, E}"/>.
    /// </summary>
    ...
    public async Task<Result<E, UseCaseErrorImpl>> ExecuteAsync(DE data)
    {
        ...
        return Result<E, UseCaseErrorImpl>.Ok(recordDeleted);
    }
    ...
    /// </remarks>
    protected virtual Task ExtraTaskAsync(DE newEntity, E createdEntity) => Task.FromResult(Unit.Value);
}
